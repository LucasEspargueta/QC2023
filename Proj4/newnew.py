...
def to_gro_line(atom):
    ...
    parts.append(format_int(atom['atom_number'], 5))
    parts.append(format_int(atom['residue_number'], 5))
...
for (mol_name, atom_name) in important_info:
    ...
    # moleculas que contém o nome (FPa, por exemplo)
    mol_df = mol_rel_df.loc[mol_rel_df['residue_name'] == mol_name]
    
    # nomes das moleculas relevantes (1FPa, 2FPa, etc)
    mol_names = pd.unique(mol_df['residue_name'])
    
    for mol_name in mol_names:
        # df que contém as linhas correspondentes aos átomos da molécula com o Grupo = mol_name
        mol_rows = mol_rel_df.loc[mol_rel_df['residue_name'] == mol_name]

        # a linha da molecula que tem Grupo = mol_name e o Átomo = atom_name
        atoms = mol_rows.loc[mol_rows['atom_name'] == atom_name]
        min_distance = 999999
        # o '_' é uma throwaway variable pois queremos o segundo elemento do tuplo, neste caso atomo
        ...
        # então avançar para a proxima molecula
